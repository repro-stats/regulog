...
`regulog` integrates with Shiny via `regulog_shiny_init()`, which:

- Resolves the **authenticated user** from `session$user` (set by Shiny Server
  Pro or Posit Connect) — not a self-reported name
- Automatically logs `session_start` and `session_end` events
- Returns a standard `regulog` object you use with the usual `log_action()` /
  `log_change()` API

## Basic pattern

```{r shiny-basic, eval = FALSE}
library(shiny)
library(regulog)
...
In development (plain `shiny::runApp()`), `session$user` is typically `NULL`.
`regulog_shiny_init()` warns and falls back to `Sys.info()[["user"]]`.
...
All sessions append to the same `.rlog` file. The NDJSON append-only format
makes this safe for concurrent writes on most file systems, but for
high-concurrency apps consider per-session files.

### Per-session log files (recommended for multi-user production)

```{r per-session-log, eval = FALSE}
log <- regulog_shiny_init(
  session = session,
  app     = "my-app",
  version = "1.0.0",
...
`regulog_shiny_init()` automatically adds two entries:
...
These bracket all user-driven entries, giving auditors a complete view of
each session.

## Using regulog_observer()

For apps with many loggable inputs, `regulog_observer()` reduces boilerplate:
...
Add a download handler to allow authorised users to export the audit trail:

```{r export-download, eval = FALSE}
