...
`log_signature()` resolves the signer identity from the session `user` and cannot be overridden per call — by design, a signature always reflects who actually opened that session, not an arbitrary name passed at signing time. Oncology safety reports typically require two independent sign-offs, so the recommended pattern is two `regulog` sessions writing to the **same `.rlog` path**: the file accumulates both signatures into one continuous hash chain.
...
# so both signatures live in one continuous, verifiable chain
log_path <- log$path
...
```{r verify-export}
# Verify the complete hash chain directly from the .rlog file --
# this covers every entry from both sessions, in one continuous chain
verify_log(log_path)

# Export for submission package
trail <- export_audit_trail(log_path,
  format = "csv",
  signed = TRUE,
  path   = file.path(tempdir(), "audit_KRASi301_safety_export_v1.csv")
)
...
- **Dual sign-off** — both the biostatistician and medical monitor signatures are part of the same continuous, verifiable chain, anchored to the same `.rlog` file
